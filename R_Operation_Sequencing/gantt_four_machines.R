gantt_four_machines = function(t0){
  limits_interval_4()
  df_limits = data.frame(starts,ends)
  library(plotrix)
  gantt.info <- list(labels = c("M1","M2","M3","M4"),#c("M1","M2"),
                   starts = starts,
                   ends = ends,
                   priorities = c(1,4))
  ...
  colores = c();for (i in 1:length(trabajos)){colores = c(colores,rep(rainbow(length(trabajos))[i],4))}
