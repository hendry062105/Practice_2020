gantt_three_machines=function(t0){
  limits_interval_3_v2()
  df_limits = data.frame(starts,ends)
  library(plotrix)
  gantt.info<-list(labels=c("M1","M2","M3"),#c("M1","M2"),
                   starts=starts,
                   ends=ends,
                   priorities=c(1,4))
  ...
  colores=c();for (i in 1:length(trabajos)){colores=c(colores,rep(rainbow(length(trabajos))[i],3))}
